// modified from https://github.com/thomaspark/scoper
...
function scoper(css, prefix) {
	...
	css = css.replace(re, function (g0, g1, g2) {
		...
		if (g1.match(/^\s*(@charset|@document|@import|@media|@namespace|@page|@supports|@.*keyframes|to|from|@font-face|1?[0-9]?[0-9])/)) {
			return g1 + g2;
		}
...
function scoperProcess() {
	...
	for (var i = 0; i < styles.length; i++) {
		...
		if (css && (style && style.parentNode.nodeName !== "BODY")) {
			var id = "scoper-" + i;
			var prefix = "#" + id;
			...
			if (parent.id) {
				throw new Error(
					"Scoped style #" + id + " is attempting to overwrite already existing id \"" +
					parent.id +
					"\" on its parent." + 
					"\n" +
					"Only use `<style scoped>` as a descendant of nodes without existing id attributes.");
			}
